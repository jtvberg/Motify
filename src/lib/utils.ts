...
export async function scrapeSpotifyTrackIds(playlistId: string): Promise<string[]> {
	try {
		const response = await fetch(`/api/scrape-spotify?id=${encodeURIComponent(playlistId)}`);
